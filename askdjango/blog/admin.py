...
class PostAdmin(admin.ModelAdmin): # Custom Admin
    list_display = ['id','title','status','created_at','tag_list']
...
class CommentAdmin(admin.ModelAdmin):
    list_display = ['id','message','post_content_len']
    # list_select_related = ['post'] # select_related 설정

    def post_content_len(self,comment):
        return '{}글자'.format(len(comment.post.content))

    def get_queryset(self, request): # select_related 설정
        qs = super().get_queryset(request)
